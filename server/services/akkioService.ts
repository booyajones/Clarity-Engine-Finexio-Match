...
class AkkioService {
  ...
  constructor() {
    // Don't require API key at construction time - check when actually needed
    this.isConfigured = !!process.env.AKKIO_API_KEY;
  ...
  async initialize(): Promise<void> {
    if (!this.isConfigured) {
      throw new Error('Akkio service is not configured - AKKIO_API_KEY environment variable is required');
    }
...
export const akkioService = process.env.AKKIO_API_KEY ? new AkkioService() : null;
