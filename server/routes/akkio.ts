...
router.get('/datasets', async (req, res) => {
  try {
    ...
    // Check if Akkio service is available
    if (!akkioService) {
      return res.json(localDatasets.map(local => ({
        ...local,
        akkio_status: 'not_configured',
        akkio_row_count: local.rowCount
      })));
    }
...
router.get('/models', async (req, res) => {
  try {
    // Check if Akkio service is available
    if (!akkioService) {
      return res.json([]);  // Return empty array when service is not configured
    }
  ...
  try {
    ...
    if (!akkioService) {
      return res.status(503).json({ 
        error: 'Akkio service is not configured', 
        details: 'AKKIO_API_KEY environment variable is required' 
      });
    }
